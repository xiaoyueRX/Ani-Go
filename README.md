...
**AutoAni** 是一个基于 Go 开发的开源番剧管理工具，支持自动追新番、历史全量补全、多下载器、多资源站，文件整理后可被 Jellyfin/fnOS 直接识别。
...
docker run -d \
  ...
  -v /your/tv/path:/TV \
  -p 8080:8080 \
  ghcr.io/xiaoyuerx/autoani:latest
```
...
```bash
# 克隆项目
git clone https://github.com/xiaoyueRX/autoani.git
cd autoani
...
| `DB_PATH` | 数据库文件路径 | `/data/autoani.db` |
