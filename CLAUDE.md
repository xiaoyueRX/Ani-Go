...
### Phase 9: v0.5.0 稳定性加固 + 插件管理闭环（2026-09-02）✅
- [x] **下载逻辑漏洞修复**：解耦 `pollRSS` 中的下载与 BangumiID 补全，确保匹配后立即下发下载任务，解决补全阻塞下载的回归问题
- [x] **多渠道元数据补偿**：实现基于 `goquery` 的 Mikan 详情页爬虫，在 API 失效时自动提取 BangumiID 和海报封面 URL
- [x] **备份恢复零值覆盖**：`RestoreBackup` 强制使用 `Select("*")` 更新，确保 `Enabled: false` 等零值字段在恢复时不会被跳过
- [x] **时间表交互加固**：`Schedule.vue` 点击跳转前增加订阅存在性校验，防止跳转至已删除订阅导致 404，提升 UI 鲁棒性
- [x] **类型断言 Panic 修复**：`internal/notifier/v2/manager.go` 下载完成通知对 `data["size"]` 误用 `.(int64)`，改为安全断言
- [x] **`StallTimeoutHours` 字段补全**：`database.Subscription` 新增字段并支持番剧级覆盖，全局默认 24h
- [x] **插件管理面板闭环**：设置页新增插件管理 Tab，支持展示状态、事件关注及热重载 (`/api/plugins/reload`)
- [x] **批量补全异步化**：`handleSupplementAll` 改为异步触发并预过滤，解决海量订阅导致的 HTTP 超时问题
...
This project is indexed by GitNexus as **Ani-Go** (4910 symbols, 9730 relationships, 169 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/Ani-Go/context` | Codebase overview, check index freshness |
| `gitnexus://repo/Ani-Go/clusters` | All functional areas |
| `gitnexus://repo/Ani-Go/processes` | All execution flows |
| `gitnexus://repo/Ani-Go/process/{name}` | Step-by-step execution trace |
