...
2. **历史全量补全 (核心特性)**: 能够爬取 Mikan 番剧详情页，自动抓取并下载全量历史种子。
...
- [x] **Phase 7: 前端美化 + 搜索修复 + 新番时间表 + 镜像测速** ✅
  - [x] Mikan 中文搜索修复（URL 编码 + CSS 选择器兼容）
  - [x] IconSax 图标组件（Iconsax Linear，20+ 图标）
  - [x] 全部视图美化（登录/侧栏/订阅/下载/设置）
  - [x] 设置页重构（分组卡片 + 配置状态 + 密码显隐）
  - [x] 新番时间表（yuc.wiki 数据源，海报图，自动刷新）
  - [x] 搜索→订阅全流程（字幕组选择 + RSS 解析）
  - [x] PWA 支持（manifest + service worker）
  - [x] 登录页记住密码
  - [x] 剧集状态手动切换
  - [x] Mikan 镜像测速（启动自动测速 + 设置页手动测速）
- [x] **Phase 9: v0.5.0 稳定性加固 + 插件管理闭环** ✅
  - [x] 下载逻辑解耦（解决补全阻塞下载问题）
  - [x] 多渠道元数据补偿（Mikan HTML 详情页爬虫）
  - [x] 备份恢复零值字段覆盖（`Select("*")`）
  - [x] 时间表跳转 404 修复与存在性校验
  - [x] 类型断言崩溃与批量操作异步化
  - [x] 插件管理 Tab 打通与 i18n 补全
- **测试**: 108 个测试全通过
