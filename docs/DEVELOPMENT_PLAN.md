...
### Phase 7: 前端美化与搜索修复 ✅
- [x] **Mikan 中文搜索修复**：`url.QueryEscape` 编码中文关键字解决 Mikan 400 错误；备选 CSS 选择器兼容 Mikan 新版页面结构。
- [x] **IconSax 图标系统**：创建 `web/src/components/IconSax.vue`，Iconsax Linear 风格 20+ 图标，全面替换 inline SVG/emoji。
- [x] **UI 全面美化**：登录页渐变背景+品牌图标、侧栏导航图标、订阅卡片 SVG 状态标识+悬停效果、下载列表状态标签图标、设置页重构。
- [x] **设置页重构**：纵向侧边栏标签、按区域分组（下载器 4 组/通知 5 组）、已配置状态 ✓ badge、密码显隐切换、配置进度指示。
