...
func TestLoad_ServerPort(t *testing.T) {
	os.Setenv("ANI_PORT", "8080")
	defer os.Unsetenv("ANI_PORT")
...
func TestLoad_DBPath(t *testing.T) {
	os.Setenv("ANI_DB_PATH", "/custom/path.db")
	defer os.Unsetenv("ANI_DB_PATH")
...
func TestLoad_DefaultDownloader(t *testing.T) {
	os.Setenv("DOWNLOADER_DEFAULT", "transmission")
	defer os.Unsetenv("DOWNLOADER_DEFAULT")
...
func TestLoad_AIConfig(t *testing.T) {
	...
	// AI 自动启用逻辑在 Load() 中通过 AI_API_KEY 是否设置来判断
	if !cfg.AI.Enabled {
		t.Logf("AI Enabled: %v", cfg.AI.Enabled)
	}
...
func TestLoad_Notifiers(t *testing.T) {
	os.Setenv("TG_BOT_TOKEN", "bot123")
	os.Setenv("TG_CHAT_ID", "chat456")
	...
	os.Setenv("LINE_TOKEN", "line-token")
...
func TestLoad_Email(t *testing.T) {
	...
	defer func() {
		for _, k := range []string{"SMTP_HOST", "EMAIL_SMTP_PORT", "EMAIL_USERNAME", "EMAIL_TO"} {
			os.Unsetenv(k)
		}
	}()
