...
func TestDatabaseVibe(t *testing.T) {
	...
	if result.BangumiID == "12345" {
		fmt.Printf("✔ 成功从数据库取回番剧，BangumiID 为: %s\n", result.BangumiID)
		fmt.Println("✔ 核心模型与字段验证通过")
	} else {
		t.Errorf("数据取回不匹配")
	}
}
